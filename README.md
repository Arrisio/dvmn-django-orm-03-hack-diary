...
## Улучшение показателей успеваимости
отредактировать скрипт `improve_schoolkid_scores.py` подставив в переменную `schoolkid_search_string` строку по которой по ФИО будет искаться ученик.
Найденному ученику будут исправлены негативные оценки, удалены замечания и добавлена одна рандомная похвала 
